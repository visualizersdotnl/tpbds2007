...
KawaseBlurBlitter::KawaseBlurBlitter(Renderer &renderer, ResourceHub &resHub) :
	Blitter(renderer),
...
void KawaseBlurBlitter::Blit(const Renderer::Texture *pTexture, uint32_t blendMode, float alpha) const
{
	TPB_ASSERT(m_pTargets[0] != NULL && pTexture != NULL);
...
void KawaseBlurBlitter::Filter(uint32_t blendMode, float alpha, bool restoreTargetValid, const Renderer::Texture *pTargetToRestore) const
{
	...
	{
		const float fPass = (float) iPass * 2.f + 1.f;
		m_renderer.SetPixelShaderConstantV(PS_USER_CI, Vector4(uvDelta.m_X * fPass, uvDelta.m_Y * fPass, 1.f, 0.f));
